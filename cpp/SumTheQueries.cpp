                  ...
                  vector<int> &E)
{
    ...
    long long int prefix_sum[n+1][m+1];
    // Fill all first col with 0
    for(int i=0; i<=n; i++)
    {  
        prefix_sum[i][0] = 0;   
    }
    // Fill all first row with 0
    for(int i=0; i<=m; i++)
    {
        prefix_sum[0][i] = 0;   
    }
    ...
    for(int i=1; i<=n; i++)
    {
        for(int j=1; j<=m; j++)
        {
            prefix_sum[i][j] = (mod + prefix_sum[i-1][j]%mod + prefix_sum[i][j-1]%mod + A[i-1][j-1]%mod - prefix_sum[i-1][j-1]%mod)%mod;   
        }
    }
    vector<int> result;
    for(int i=0; i<B.size(); i++)
    {
        ...
        int sum = (mod + prefix_sum[brr][brc]%mod - prefix_sum[tlr - 1][brc]%mod - prefix_sum[brr][tlc - 1]%mod + prefix_sum[tlr - 1][tlc - 1]%mod)%mod;
...
vector<int> solve2(vector<vector<int> > &A, vector<int> &B, vector<int> &C, vector<int> &D, vector<int> &E) {
    long long int mod_value = 10000000007;
    vector<vector<long long int>> prefix_sum(A.size(), vector<long long int>(A[0].size(), 0));
    for (int i = 0; i < prefix_sum.size(); i++)
    {
        for (int j = 0; j < prefix_sum[i].size(); j++)
        {
            ...
            else
            {
                ...
                if (j - 1 >= 0)
                {
                    sum = prefix_sum[i][j - 1]% mod_value;
                }
                if (i - 1 >= 0)
                {
                    sum = (sum + prefix_sum[i - 1][j]) % mod_value;
                }
                if (j - 1 >= 0 && i - 1 >= 0)
                {
                    sum = (sum - prefix_sum[i - 1][j - 1]) % mod_value;
                }
                sum = (sum + A[i][j]) % mod_value;
                prefix_sum[i][j] = sum;
            }
        }
    }
    ...
    for (int i = 0; i < B.size(); i++)
    {
        ...
        if (tl_c - 1 >= 0)
        {
            sum = (prefix_sum[br_r][br_c] - prefix_sum[br_r][tl_c - 1]) % mod_value;
        }
        else
        {
            sum = (prefix_sum[br_r][br_c]) % mod_value;
        }
        if (tl_r - 1 >= 0)
        {
            sum = (sum - prefix_sum[tl_r - 1][br_c]) % mod_value;
        }
        if (tl_r - 1 >= 0 && tl_c - 1 >= 0)
        {
            sum = (sum + prefix_sum[tl_r - 1][tl_c - 1]) % mod_value;
        }
...
int main()
{
    ...
    vector<int> B{14, 12, 7, 7, 18, 13, 6, 18, 1, 33, 4, 7, 11, 16, 27, 9, 4, 8, 25, 25, 11, 34, 21, 6, 20, 1, 4, 16, 19};
    vector<int> C{18, 8, 9, 11, 6, 4, 17, 7, 3, 1, 2, 13, 19, 4, 20, 8, 6, 1, 13, 1, 2, 25, 25, 11, 1, 23, 14, 18, 19};
    vector<int> D{18, 35, 11, 12, 30, 39, 40, 19, 3, 37, 37, 44, 43, 30, 29, 17, 37, 29, 34, 41, 14, 38, 45, 30, 41, 24, 14, 29, 20};
    vector<int> E{20, 19, 24, 22, 19, 4, 28, 13, 16, 9, 23, 13, 28, 21, 21, 21, 14, 18, 28, 4, 4, 27, 27, 14, 29, 25, 19, 28, 20};
